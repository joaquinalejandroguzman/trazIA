...
const GENERATE_SPEC_TIMEOUT_MS = 30000;
...
export function useAnalysis() {
    ...
    const [generatingSpec, setGeneratingSpec] = useState(null);
    const [specErrorModules, setSpecErrorModules] = useState(new Set());
    ...
    const generateSpec = useCallback(async (moduleId) => {
        ...
        if (specErrorModules.has(moduleId))
            return null;
...
**THE** sistema **SHALL** manejar timeouts y errores de red con reintentos`;
            ...
            setResult((prev) => {
                if (!prev)
                    return prev;
                const updatedModules = prev.modules.map((m) => m.id === moduleId ? { ...m, earsSpec: mockSpec } : m);
        ...
        try {
            const response = await apiClient.post('/api/generate-spec', { moduleId, moduleName: module.name, sourceContent: module.sourceContent }, { signal: controller.signal, _skipRetry: true });
            clearTimeout(timeoutId);
            // Éxito: actualizar solo module.earsSpec, NO modificar specStatus ni specHealthScore
            setResult((prev) => {
                if (!prev)
                    return prev;
                const updatedModules = prev.modules.map((m) => m.id === moduleId ? { ...m, earsSpec: response.data.earsSpec } : m);
                return { ...prev, modules: updatedModules };
            });
            ...
            const message = err instanceof Error ? err.message : 'Error al generar spec';
            setError(message);
            return null;
        }
        finally {
    ...
    const clearSpecError = useCallback((moduleId) => {
