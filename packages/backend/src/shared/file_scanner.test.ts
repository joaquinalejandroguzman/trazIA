...
describe('scanAllFiles', () => {
  ...
  it('includes custom dev code from static/ directories', () => {
    // Crear carpeta static/ con un service worker custom
    fs.mkdirSync(path.join(tmpDir, 'public'))
    fs.writeFileSync(
      path.join(tmpDir, 'public', 'sw.js'),
      '// Service worker\nself.addEventListener("fetch", (e) => {\n  e.respondWith(caches.match(e.request))\n})\n'
    )
    // Crear un vendor minificado que debería excluirse
    fs.writeFileSync(
      path.join(tmpDir, 'public', 'jquery.min.js'),
      'x'.repeat(25000) // > 20KB
    )
    // Crear un archivo binario que debería excluirse
    fs.writeFileSync(path.join(tmpDir, 'public', 'logo.png'), 'fake-png')

    const files = scanAllFiles(tmpDir, tmpDir)
    const rel = files.map(f => path.relative(tmpDir, f).replace(/\\/g, '/'))
    expect(rel).toContain('public/sw.js')
    expect(rel).not.toContain('public/jquery.min.js')
    expect(rel).not.toContain('public/logo.png')
  })

  it('excludes vendor scripts from static/ by name pattern', () => {
    fs.mkdirSync(path.join(tmpDir, 'static'))
    fs.writeFileSync(
      path.join(tmpDir, 'static', 'analytics.js'),
      '// Google Analytics\nwindow.ga = function() {}\n'
    )
    fs.writeFileSync(
      path.join(tmpDir, 'static', 'init.js'),
      '// Inicialización custom\nconst app = { start() {} }\n'
    )

    const files = scanAllFiles(tmpDir, tmpDir)
    const rel = files.map(f => path.relative(tmpDir, f).replace(/\\/g, '/'))
    expect(rel).not.toContain('static/analytics.js')
    expect(rel).toContain('static/init.js')
  })

  it('excludes minified content from static/ even if filename looks normal', () => {
    fs.mkdirSync(path.join(tmpDir, 'assets'))
    // Archivo con líneas extremadamente largas (minificado)
    const minified = 'var a=' + 'x'.repeat(300) + ';var b=' + 'y'.repeat(300) + ';\n'
    fs.writeFileSync(path.join(tmpDir, 'assets', 'app.js'), minified)

    const files = scanAllFiles(tmpDir, tmpDir)
    const rel = files.map(f => path.relative(tmpDir, f).replace(/\\/g, '/'))
    expect(rel).not.toContain('assets/app.js')
  })
})
